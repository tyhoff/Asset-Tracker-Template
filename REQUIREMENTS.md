...
- **Known limitation, accepted for this pass:** there is no session header. `struct
  ...
  tested, but nothing calls them on this path — emitting one needs a modem-version accessor this
  build has no plumbing for yet (`modem_info_init()` + a call site where the modem is guaranteed
  responsive, which rules out boot-time `SYS_INIT`). The export is a bare stream of records; a
  session header is a follow-up, not a CP8 blocker.
